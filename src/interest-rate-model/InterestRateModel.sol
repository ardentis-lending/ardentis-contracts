...
import { Id, MarketParams, Market } from "ardentis/interfaces/IArdentis.sol";
import { MathLib as ArdentisMathLib } from "ardentis/libraries/MathLib.sol";
...
contract InterestRateModel is UUPSUpgradeable, AccessControlEnumerableUpgradeable, IInterestRateModel {
  ...
  using ArdentisMathLib for uint128;
  ...
  function borrowRate(MarketParams memory marketParams, Market memory market) external returns (uint256) {
    require(msg.sender == ARDENTIS, ErrorsLib.NOT_ARDENTIS);
  ...
  function _borrowRate(Id id, Market memory market) private view returns (uint256, int256) {
    ...
    if (startRateAtTarget == 0) {
      ...
      if (linearAdaptation == 0) {
        ...
        // Formula of the average rate that should be returned to Ardentis:
