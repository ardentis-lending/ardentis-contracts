...
import { IArdentis, Id, MarketParams } from "ardentis/interfaces/IArdentis.sol";
...
/// @dev This interface is used for factorizing IArdentisVaultStaticTyping and IArdentisVault.
/// @dev Consider using the IArdentisVault interface instead of this one.
interface IArdentisVaultBase {
  /// @notice The address of the Ardentis contract.
  function ARDENTIS() external view returns (IArdentis);
...
/// @dev This interface is inherited by ArdentisVault so that function signatures are checked by the compiler.
/// @dev Consider using the IArdentisVault interface instead of this one.
interface IArdentisVaultStaticTyping is IArdentisVaultBase {
  /// @notice Returns the current configuration of each market.
  function config(Id) external view returns (uint184 cap, bool enabled, uint64 removableAt);
}

/// @title IArdentisVault
/// @author Ardentis
/// @dev Use this interface for ArdentisVault to have access to all the functions with the appropriate function signatures.
interface IArdentisVault is IArdentisVaultBase, IERC4626, IERC20Permit, IOwnable, IMulticall {
  /// @notice Returns the current configuration of each market.
