...
import { MarketConfig, PendingUint192, PendingAddress, MarketAllocation, IArdentisVaultBase, IArdentisVaultStaticTyping } from "./interfaces/IArdentisVault.sol";
import { Id, MarketParams, Market, IArdentis } from "ardentis/interfaces/IArdentis.sol";

import { PendingUint192, PendingAddress, PendingLib } from "./libraries/PendingLib.sol";
import { ConstantsLib } from "./libraries/ConstantsLib.sol";
import { ErrorsLib } from "./libraries/ErrorsLib.sol";
import { EventsLib } from "./libraries/EventsLib.sol";
import { WAD } from "ardentis/libraries/MathLib.sol";
...
/// @title ArdentisVault
/// @author Ardentis
/// @notice ERC4626 compliant vault allowing users to deposit assets to Ardentis.
contract ArdentisVault is
  UUPSUpgradeable,
  AccessControlEnumerableUpgradeable,
  ERC4626Upgradeable,
  ERC20PermitUpgradeable,
  IArdentisVaultStaticTyping
{
  ...
  /// @inheritdoc IArdentisVaultStaticTyping
  ...
  constructor(address ardentis, address _asset) {
  ...
  function initialize(
    ...
    IERC20(_asset).forceApprove(address(ARDENTIS), type(uint256).max);
  }
  ...
  function setCap(MarketParams memory marketParams, uint256 newSupplyCap) external onlyRole(CURATOR) {
    ...
    if (ARDENTIS.market(id).lastUpdate == 0) revert ErrorsLib.MarketNotCreated();
  ...
  function updateWithdrawQueue(uint256[] calldata indexes) external onlyRole(ALLOCATOR) {
    ...
    for (uint256 i; i < currLength; ++i) {
      if (!seen[i]) {
        ...
        if (ARDENTIS.position(id, address(this)).supplyShares != 0) {
  ...
  function reallocate(MarketAllocation[] calldata allocations) external onlyAllocatorOrBot {
    ...
    for (uint256 i; i < allocations.length; ++i) {
      ...
      if (withdrawn > 0) {
        ...
        (uint256 withdrawnAssets, uint256 withdrawnShares) = ARDENTIS.withdraw(
        ...
        (, uint256 suppliedShares) = ARDENTIS.supply(allocation.marketParams, suppliedAssets, 0, address(this), hex"");
  ...
  function totalAssets() public view override returns (uint256 assets) {
    for (uint256 i; i < withdrawQueue.length; ++i) {
      assets += ARDENTIS.expectedSupplyAssets(_marketParams(withdrawQueue[i]), address(this));
    }
  }
  ...
  function _maxWithdraw(
    ...
    assets -= _simulateWithdrawArdentis(assets);
  }

  /// @dev Returns the maximum amount of assets that the vault can supply on Ardentis.
  function _maxDeposit() internal view returns (uint256 totalSuppliable) {
    for (uint256 i; i < supplyQueue.length; ++i) {
      ...
      uint256 supplyShares = ARDENTIS.position(id, address(this)).supplyShares;
      (uint256 totalSupplyAssets, uint256 totalSupplyShares, , ) = ARDENTIS.expectedMarketBalances(_marketParams(id));
  ...
  /// @inheritdoc ERC4626Upgradeable
  /// @dev Used in mint or deposit to deposit the underlying asset to Ardentis markets.
  function _deposit(address caller, address receiver, uint256 assets, uint256 shares) internal override {
    super._deposit(caller, receiver, assets, shares);

    _supplyArdentis(assets);
  ...
  /// @inheritdoc ERC4626Upgradeable
  /// @dev Used in redeem or withdraw to withdraw the underlying asset from Ardentis markets.
  ...
  function _marketParams(Id id) internal view returns (MarketParams memory) {
    return ARDENTIS.idToMarketParams(id);
  }

  /// @dev Accrues interest on Ardentis and returns the vault's assets & corresponding shares supplied on the
  ...
  function _accruedSupplyBalance(
    ...
    market = ARDENTIS.market(id);
    shares = ARDENTIS.position(id, address(this)).supplyShares;
  ...
  function _setCap(MarketParams memory marketParams, Id id, uint184 supplyCap) internal {
    ...
    if (supplyCap > 0) {
      if (!marketConfig.enabled) {
        ...
        _updateLastTotalAssets(lastTotalAssets + ARDENTIS.expectedSupplyAssets(marketParams, address(this)));
  ...
  /// @dev Supplies `assets` to Ardentis.
  function _supplyArdentis(uint256 assets) internal {
    for (uint256 i; i < supplyQueue.length; ++i) {
      Id id = supplyQueue[i];
      ...
      if (toSupply > 0) {
        ...
        try ARDENTIS.supply(marketParams, toSupply, 0, address(this), hex"") {
          assets -= toSupply;
        } catch {}
      }
  ...
  /// @dev Withdraws `assets` from Ardentis.
  function _withdrawArdentis(uint256 assets) internal {
    for (uint256 i; i < withdrawQueue.length; ++i) {
      Id id = withdrawQueue[i];
      MarketParams memory marketParams = _marketParams(id);
      ...
      if (toWithdraw > 0) {
        // Using try/catch to skip markets that revert.
        try ARDENTIS.withdraw(marketParams, toWithdraw, 0, address(this), address(this)) {
          assets -= toWithdraw;
        } catch {}
      }
  ...
  /// @dev Simulates a withdraw of `assets` from Ardentis.
  /// @return The remaining assets to be withdrawn.
  function _simulateWithdrawArdentis(uint256 assets) internal view returns (uint256) {
    for (uint256 i; i < withdrawQueue.length; ++i) {
      Id id = withdrawQueue[i];
      MarketParams memory marketParams = _marketParams(id);

      uint256 supplyShares = ARDENTIS.position(id, address(this)).supplyShares;
      (uint256 totalSupplyAssets, uint256 totalSupplyShares, uint256 totalBorrowAssets, ) = ARDENTIS
        .expectedMarketBalances(marketParams);

      // The vault withdrawing from Ardentis cannot fail because:
      // 1. oracle.price() is never called (the vault doesn't borrow)
      // 2. the amount is capped to the liquidity available on Ardentis
  ...
  function _withdrawable(
    ...
    // Inside a flashloan callback, liquidity on Ardentis may be limited to the singleton's balance.
    uint256 availableLiquidity = UtilsLib.min(
      totalSupplyAssets - totalBorrowAssets,
      ERC20Upgradeable(marketParams.loanToken).balanceOf(address(ARDENTIS))
    );
