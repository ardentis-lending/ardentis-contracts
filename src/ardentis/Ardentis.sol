...
import { Id, IArdentisStaticTyping, IArdentisBase, MarketParams, Position, Market, Authorization, Signature } from "./interfaces/IArdentis.sol";
import { IArdentisLiquidateCallback, IArdentisRepayCallback, IArdentisSupplyCallback, IArdentisSupplyCollateralCallback, IArdentisFlashLoanCallback } from "./interfaces/IArdentisCallbacks.sol";
...
/// @title Ardentis
/// @author Ardentis
/// @notice The Ardentis contract.
contract Ardentis is
  UUPSUpgradeable,
  AccessControlEnumerableUpgradeable,
  ReentrancyGuardUpgradeable,
  PausableUpgradeable,
  IArdentisStaticTyping
{
  ...
  /// @inheritdoc IArdentisStaticTyping
  ...
  function supply(
    ...
    if (data.length > 0) IArdentisSupplyCallback(msg.sender).onArdentisSupply(assets, data);
  ...
  function repay(
    ...
    if (data.length > 0) IArdentisRepayCallback(msg.sender).onArdentisRepay(assets, data);
  ...
  function supplyCollateral(
    ...
    if (data.length > 0) IArdentisSupplyCollateralCallback(msg.sender).onArdentisSupplyCollateral(assets, data);
  ...
  function liquidate(
    ...
    if (data.length > 0) IArdentisLiquidateCallback(msg.sender).onArdentisLiquidate(repaidAssets, data);
  ...
  function flashLoan(address token, uint256 assets, bytes calldata data) external whenNotPaused {
    ...
    IArdentisFlashLoanCallback(msg.sender).onArdentisFlashLoan(assets, data);
