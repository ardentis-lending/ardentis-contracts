...
/// @dev This interface is used for factorizing IArdentisStaticTyping and IArdentis.
/// @dev Consider using the IArdentis interface instead of this one.
interface IArdentisBase {
  ...
  /// Ardentis behaves as expected:
  /// - The token should be ERC-20 compliant, except that it can omit return values on `transfer` and `transferFrom`.
  /// - The token balance of Ardentis should only decrease on `transfer` and `transferFrom`. In particular, tokens with
  /// burn functions are not supported.
  /// - The token should not re-enter Ardentis on `transfer` nor `transferFrom`.
  /// - The token balance of the sender (resp. receiver) should decrease (resp. increase) by exactly the given amount
  /// on `transfer` and `transferFrom`. In particular, tokens with fees on transfer are not supported.
  /// - The IRM should not re-enter Ardentis.
  /// - The oracle should return a price with the correct scaling.
  /// @dev Here is a list of properties on the market's dependencies that could break Ardentis's liveness properties
  ...
  /// `onArdentisSupply` function with the given `data`.
  ...
  /// @param data Arbitrary data to pass to the `onArdentisSupply` callback. Pass empty data if not needed.
  ...
  /// `onArdentisRepay` function with the given `data`.
  ...
  /// @param data Arbitrary data to pass to the `onArdentisRepay` callback. Pass empty data if not needed.
  ...
  /// `onArdentisSupplyCollateral` function with the given `data`.
  /// @dev Interest are not accrued since it's not required and it saves gas.
  /// @dev Supplying a large amount can revert for overflow.
  /// @param marketParams The market to supply collateral to.
  /// @param assets The amount of collateral to supply.
  /// @param onBehalf The address that will own the increased collateral position.
  /// @param data Arbitrary data to pass to the `onArdentisSupplyCollateral` callback. Pass empty data if not needed.
  ...
  /// `onArdentisLiquidate` function with the given `data`.
  ...
  /// @param data Arbitrary data to pass to the `onArdentisLiquidate` callback. Pass empty data if not needed.
  ...
  /// @param data Arbitrary data to pass to the `onArdentisFlashLoan` callback.
...
/// @dev This interface is inherited by Ardentis so that function signatures are checked by the compiler.
/// @dev Consider using the IArdentis interface instead of this one.
interface IArdentisStaticTyping is IArdentisBase {
  ...
  /// @dev This mapping is not used in Ardentis. It is there to enable reducing the cost associated to calldata on layer
...
/// @title IArdentis
/// @author Ardentis
/// @dev Use this interface for Ardentis to have access to all the functions with the appropriate function signatures.
interface IArdentis is IArdentisBase {
  /// @notice The state of the position of `user` on the market corresponding to `id`.
