...
import { Id, MarketParams, Market, IArdentis } from "../../interfaces/IArdentis.sol";
...
/// @title ArdentisBalancesLib
/// @author Ardentis
/// @notice Helper library exposing getters with the expected value after interest accrual.
/// @dev This library is not used in Ardentis itself and is intended to be used by integrators.
/// @dev The getter to retrieve the expected total borrow shares is not exposed because interest accrual does not apply
/// to it. The value can be queried directly on Ardentis using `totalBorrowShares`.
library ArdentisBalancesLib {
  ...
  function expectedTotalSupplyAssets(
    ...
    (totalSupplyAssets, , , ) = expectedMarketBalances(ardentis, marketParams);
  }
  ...
  function expectedTotalBorrowAssets(
    ...
    (, , totalBorrowAssets, ) = expectedMarketBalances(ardentis, marketParams);
  }
  ...
  function expectedTotalSupplyShares(
    ...
    (, totalSupplyShares, , ) = expectedMarketBalances(ardentis, marketParams);
  }
  ...
  function expectedSupplyAssets(
    ...
    Id id = marketParams.id();
    uint256 supplyShares = ardentis.position(id, user).supplyShares;
    (uint256 totalSupplyAssets, uint256 totalSupplyShares, , ) = expectedMarketBalances(ardentis, marketParams);
  ...
  function expectedBorrowAssets(
    ...
    Id id = marketParams.id();
    uint256 borrowShares = ardentis.position(id, user).borrowShares;
    (, , uint256 totalBorrowAssets, uint256 totalBorrowShares) = expectedMarketBalances(ardentis, marketParams);
