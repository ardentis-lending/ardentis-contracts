...
contract BatchManagementUtils is AccessControlEnumerableUpgradeable, UUPSUpgradeable {
  ...
  constructor(address ardentis) {
    require(ardentis != address(0), "ardentis is zero address");
   ...
   *
   * Requirements:
   * - Caller must have the MANAGER role for the specified `ardentis` contract.
   * - `ids` and `fees` arrays must have the same length and be non-empty.
   * - Each market ID in `ids` must correspond to an existing market in the `ardentis` contract.
   */
  function batchSetMarketFee(Id[] calldata ids, uint256[] calldata fees) external {
    require(IAccessControl(ARDENTIS).hasRole(MANAGER, msg.sender), "Not manager of ardentis");
    require(ids.length == fees.length && ids.length > 0, "Array length mismatch");

    for (uint256 i = 0; i < ids.length; i++) {
      MarketParams memory marketParams = IArdentis(ARDENTIS).idToMarketParams(ids[i]);
      require(marketParams.loanToken != address(0), "Market not created");
      IArdentis(ARDENTIS).setFee(marketParams, fees[i]);
    }
  }
