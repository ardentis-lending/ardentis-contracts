...
contract ResilientOracle is OwnableUpgradeable, UUPSUpgradeable, OracleInterface {
  ...
  function initialize(BoundValidatorInterface _boundValidator, address _owner) public initializer {
  ...
  function _getPrice(address asset) internal view returns (uint256) {
    ...
    if (pivotOracleEnabled && pivotOracle != address(0)) {
      try this.getPriceFromOracle(pivotOracle, tokenConfigs[asset].timeDeltaTolerance) returns (uint256 pricePivot) {
  ...
  function _getMainOraclePrice(
    ...
    if (mainOracleEnabled && mainOracle != address(0)) {
      try this.getPriceFromOracle(mainOracle, tokenConfigs[asset].timeDeltaTolerance) returns (
        ...
        return (mainOraclePrice, boundValidator.validatePriceWithAnchorPrice(asset, mainOraclePrice, pivotPrice));
      } catch {
  ...
  function _getFallbackOraclePrice(address asset, uint256 pivotPrice) private view returns (uint256, bool) {
    ...
    if (fallbackEnabled && fallbackOracle != address(0)) {
      try this.getPriceFromOracle(fallbackOracle, tokenConfigs[asset].timeDeltaTolerance) returns (
