...
import { MathLib, WAD } from "ardentis/libraries/MathLib.sol";
import { SharesMathLib } from "ardentis/libraries/SharesMathLib.sol";
import "ardentis/libraries/ConstantsLib.sol";
...
contract PublicLiquidator is UUPSUpgradeable, AccessControlEnumerableUpgradeable, IPublicLiquidator {
  using MarketParamsLib for IArdentis.MarketParams;
  ...
  constructor(address ardentis) {
    require(ardentis != address(0), ZERO_ADDRESS);
  ...
  function setMarketWhitelist(bytes32 id, bool status) external onlyRole(BOT) {
    require(IArdentis(ARDENTIS).idToMarketParams(id).loanToken != address(0), "Invalid market");
    require(!IArdentis(ARDENTIS).isLiquidationWhitelist(id, address(0)), "market is already open for liquidate");
  ...
  function setMarketUserWhitelist(bytes32 id, address user, bool status) external onlyRole(BOT) {
    require(IArdentis(ARDENTIS).idToMarketParams(id).loanToken != address(0), "Invalid market");
    require(
      !marketWhitelist[id] || !IArdentis(ARDENTIS).isLiquidationWhitelist(id, address(0)),
  ...
  function flashLiquidate(
    ...
    IArdentis.MarketParams memory params = IArdentis(ARDENTIS).idToMarketParams(id);
    ...
    IArdentis(ARDENTIS).liquidate(
      params,
      borrower,
      seizedAssets,
      0,
      abi.encode(
        ArdentisLiquidateData(
  ...
  function flashLiquidateSmartCollateral(
    ...
    uint256 loanTokenBalanceBefore = SafeTransferLib.balanceOf(params.loanToken, address(this));
    // liquidate borrower's position
    ArdentisLiquidateData memory callback = ArdentisLiquidateData(
      params.collateralToken,
      params.loanToken,
      seizedAssets,
      address(0),
    ...
    (uint256 _seizedAssets, uint256 _repaidAssets) = IArdentis(ARDENTIS).liquidate(
      params,
      borrower,
      seizedAssets,
      0,
  ...
  function liquidate(bytes32 id, address borrower, uint256 seizedAssets, uint256 repaidShares) external {
    ...
    IArdentis(ARDENTIS).accrueInterest(params);
  ...
  ///      exactly the same logic as in the Ardentis contract.
  /// @param id The id of the market.
  ...
  function loanTokenAmountNeed(bytes32 id, uint256 seizedAssets, uint256 repaidShares) public view returns (uint256) {
    IArdentis.MarketParams memory params = IArdentis(ARDENTIS).idToMarketParams(id);
    IArdentis.Market memory market = IArdentis(ARDENTIS).market(id);
    ...
    if (seizedAssets > 0) {
      ...
      uint256 collateralPrice = IArdentis(ARDENTIS).getPrice(params);
  ...
  /// @param id The id of the market.
  /// @param borrower The address of the borrower.
  function postLiquidate(IArdentis.MarketParams memory params, bytes32 id, address borrower) internal {
    // remove user from whitelist if position is healthy and inside whitelist
    if (IArdentis(ARDENTIS).isHealthy(params, id, borrower) && marketUserWhitelist[id][borrower]) {
  ...
  function isLiquidatable(bytes32 id, address borrower) internal view returns (bool) {
    return
      IArdentis(ARDENTIS).isLiquidationWhitelist(id, address(0)) ||
      marketWhitelist[id] ||
      marketUserWhitelist[id][borrower];
  }

  /// @dev the function will be called by the Ardentis contract when liquidate.
  ...
  function onArdentisLiquidate(uint256 repaidAssets, bytes calldata data) external {
    require(msg.sender == ARDENTIS, OnlyArdentis());
    ArdentisLiquidateData memory arb = abi.decode(data, (ArdentisLiquidateData));
    ...
    SafeTransferLib.safeApprove(arb.loanToken, ARDENTIS, repaidAssets);
  }
