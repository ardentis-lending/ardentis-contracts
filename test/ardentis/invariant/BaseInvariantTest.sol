...
contract BaseInvariantTest is InvariantTest {
  ...
  function setUp() public virtual override {
    ...
    for (uint256 i = 2; i <= 6; ++i) {
      ...
      vm.startPrank(OWNER);
      ardentis.enableLltv(_marketParams.lltv);
      ardentis.createMarket(_marketParams);
      vm.stopPrank();
  ...
  modifier authorized(address onBehalf) {
    if (onBehalf != msg.sender && !ardentis.isAuthorized(onBehalf, msg.sender)) {
      vm.prank(onBehalf);
      ardentis.setAuthorization(msg.sender, true);
    }

    _;

    if (ardentis.isAuthorized(onBehalf, msg.sender)) {
      vm.prank(onBehalf);
      ardentis.setAuthorization(msg.sender, false);
    }
  }
  ...
  function _supplyAssets(
    ...
    vm.prank(msg.sender);
    ardentis.supply(_marketParams, assets, 0, onBehalf, hex"");
  }
  ...
  function _supplyShares(
    ...
    (uint256 totalSupplyAssets, uint256 totalSupplyShares, , ) = ardentis.expectedMarketBalances(_marketParams);

    loanToken.setBalance(msg.sender, shares.toAssetsUp(totalSupplyAssets, totalSupplyShares));

    vm.prank(msg.sender);
    ardentis.supply(_marketParams, 0, shares, onBehalf, hex"");
  }
  ...
  function _withdraw(
    ...
    vm.prank(msg.sender);
    ardentis.withdraw(_marketParams, assets, shares, onBehalf, receiver);
  }
  ...
  function _borrow(
    ...
    vm.prank(msg.sender);
    ardentis.borrow(_marketParams, assets, shares, onBehalf, receiver);
  }
  ...
  function _repayAssets(
    ...
    vm.prank(msg.sender);
    ardentis.repay(_marketParams, assets, 0, onBehalf, hex"");
  }
  ...
  function _repayShares(
    ...
    vm.prank(msg.sender);
    ardentis.repay(_marketParams, 0, shares, onBehalf, hex"");
  }
  ...
  function _supplyCollateral(
    ...
    vm.prank(msg.sender);
    ardentis.supplyCollateral(_marketParams, assets, onBehalf, hex"");
  }
  ...
  function _withdrawCollateral(
    ...
    vm.prank(msg.sender);
    ardentis.withdrawCollateral(_marketParams, assets, onBehalf, receiver);
  }
  ...
  function _liquidateSeizedAssets(
    ...
    vm.prank(msg.sender);
    ardentis.liquidate(_marketParams, borrower, seizedAssets, 0, hex"");
  }
  ...
  function _liquidateRepaidShares(
    ...
    vm.prank(msg.sender);
    ardentis.liquidate(_marketParams, borrower, 0, repaidShares, hex"");
  }
  ...
  function setFeeNoRevert(uint256 marketSeed, uint256 newFee) external {
    ...
    if (newFee == ardentis.market(_id).fee) return;

    vm.prank(OWNER);
    ardentis.setFee(_marketParams, newFee);
  }
