...
contract InvariantTest is BaseTest {
  ...
  function _randomSupplier(
    ...
    for (uint256 i; i < users.length; ++i) {
      ...
      if (ardentis.position(_id, user).supplyShares != 0) {
        candidates[i] = user;
      }
    }
  ...
  function _randomBorrower(
    ...
    for (uint256 i; i < users.length; ++i) {
      ...
      if (ardentis.position(_id, user).borrowShares != 0) {
        candidates[i] = user;
      }
    }
  ...
  function _randomHealthyCollateralSupplier(
    ...
    for (uint256 i; i < users.length; ++i) {
      ...
      if (ardentis.position(_id, user).collateral != 0 && _isHealthy(_marketParams, user)) {
        candidates[i] = user;
      }
    }
