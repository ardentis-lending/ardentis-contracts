...
contract WithdrawCollateralIntegrationTest is BaseTest {
  ...
  function testWithdrawCollateralMarketNotCreated(MarketParams memory marketParamsFuzz) public {
    ...
    ardentis.withdrawCollateral(marketParamsFuzz, 1, SUPPLIER, RECEIVER);
  }
  ...
  function testWithdrawCollateralZeroAmount(uint256 amount) public {
    ...
    ardentis.supplyCollateral(marketParams, amount, SUPPLIER, hex"");

    vm.expectRevert(bytes(ErrorsLib.ZERO_ASSETS));
    ardentis.withdrawCollateral(marketParams, 0, SUPPLIER, RECEIVER);
    vm.stopPrank();
  }
  ...
  function testWithdrawCollateralToZeroAddress(uint256 amount) public {
    ...
    ardentis.withdrawCollateral(marketParams, amount, SUPPLIER, address(0));
    vm.stopPrank();
  }
  ...
  function testWithdrawCollateralUnauthorized(address attacker, uint256 amount) public {
    ...
    ardentis.withdrawCollateral(marketParams, amount, SUPPLIER, RECEIVER);
  }
  ...
  function testWithdrawCollateralUnhealthyPosition(
    ...
    ardentis.withdrawCollateral(marketParams, amountCollateral, BORROWER, BORROWER);
    vm.stopPrank();
  }
  ...
  function testWithdrawCollateral(
    ...
    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(marketParams, amountCollateral + amountCollateralExcess, BORROWER, hex"");
    ardentis.borrow(marketParams, amountBorrowed, 0, BORROWER, BORROWER);
    ...
    ardentis.withdrawCollateral(marketParams, amountCollateralExcess, BORROWER, RECEIVER);

    vm.stopPrank();

    assertEq(ardentis.position(id, BORROWER).collateral, amountCollateral, "collateral balance");
    assertEq(collateralToken.balanceOf(RECEIVER), amountCollateralExcess, "lender balance");
    assertEq(collateralToken.balanceOf(address(ardentis)), amountCollateral, "ardentis balance");
  }
  ...
  function testWithdrawCollateralOnBehalf(
    ...
    ardentis.supplyCollateral(marketParams, amountCollateral + amountCollateralExcess, ONBEHALF, hex"");
    // BORROWER is already authorized.
    ardentis.borrow(marketParams, amountBorrowed, 0, ONBEHALF, ONBEHALF);
    vm.stopPrank();
    ...
    ardentis.withdrawCollateral(marketParams, amountCollateralExcess, ONBEHALF, RECEIVER);

    assertEq(ardentis.position(id, ONBEHALF).collateral, amountCollateral, "collateral balance");
    assertEq(collateralToken.balanceOf(RECEIVER), amountCollateralExcess, "lender balance");
    assertEq(collateralToken.balanceOf(address(ardentis)), amountCollateral, "ardentis balance");
  }
}
