...
contract LiquidateIntegrationTest is BaseTest {
  ...
  function testLiquidateNotCreatedMarket(MarketParams memory marketParamsFuzz, uint256 lltv) public {
    ...
    ardentis.liquidate(marketParamsFuzz, address(this), 1, 0, hex"");
  }
  ...
  function testLiquidateZeroAmount(uint256 lltv) public {
    ...
    ardentis.liquidate(marketParams, address(this), 0, 0, hex"");
  }
  ...
  function testLiquidateInconsistentInput(uint256 seized, uint256 sharesRepaid) public {
    ...
    vm.expectRevert(bytes(ErrorsLib.INCONSISTENT_INPUT));
    ardentis.liquidate(marketParams, address(this), seized, sharesRepaid, hex"");
  }
  ...
  function testLiquidateHealthyPosition(
    ...
    ardentis.liquidate(marketParams, BORROWER, amountSeized, 0, hex"");
  }
  ...
  function testLiquidateMargin(LiquidateTestParams memory params, uint256 amountSeized, uint256 elapsed) public {
    ...
    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(marketParams, params.amountCollateral, BORROWER, hex"");
    ardentis.borrow(marketParams, params.amountBorrowed, 0, BORROWER, BORROWER);
    vm.stopPrank();

    // We have to estimate the ratio after borrowing because the borrow rate depends on the utilization.
    uint256 maxRatio = WAD + irm.borrowRate(marketParams, ardentis.market(id)).wTaylorCompounded(elapsed);
  ...
  function testLiquidateSeizedInputNoBadDebtRealized(LiquidateTestParams memory params, uint256 amountSeized) public {
    ...
    uint256 borrowShares = ardentis.position(id, BORROWER).borrowShares;
    ...
    uint256 expectedRepaidShares = expectedRepaid.toSharesDown(
      ...
      ardentis.market(id).totalBorrowShares
    );
    ...
    (uint256 returnSeized, uint256 returnRepaid) = ardentis.liquidate(marketParams, BORROWER, amountSeized, 0, hex"");
    ...
    assertEq(ardentis.market(id).totalBorrowAssets, expectedBorrowed, "total borrow");
    assertEq(ardentis.market(id).totalBorrowShares, expectedBorrowShares, "total borrow shares");
    assertEq(ardentis.position(id, BORROWER).collateral, expectedCollateral, "collateral");
    ...
    assertEq(loanToken.balanceOf(address(ardentis)), params.amountSupplied - expectedBorrowed, "ardentis balance");
    assertEq(collateralToken.balanceOf(address(ardentis)), expectedCollateral, "ardentis collateral balance");
  ...
  function testLiquidateSharesInputNoBadDebtRealized(LiquidateTestParams memory params, uint256 sharesRepaid) public {
    ...
    uint256 maxSharesRepaid = (params.amountCollateral - 1)
      ...
      .toSharesDown(ardentis.market(id).totalBorrowAssets, ardentis.market(id).totalBorrowShares);
    ...
    uint256 expectedSeized = sharesRepaid
      .toAssetsDown(ardentis.market(id).totalBorrowAssets, ardentis.market(id).totalBorrowShares)
    ...
    (uint256 returnSeized, uint256 returnRepaid) = ardentis.liquidate(marketParams, BORROWER, 0, sharesRepaid, hex"");
  ...
  function testLiquidateBadDebtRealized(
    ...
    params.borrowSharesBeforeLiquidation = ardentis.position(id, BORROWER).borrowShares;
    params.totalBorrowSharesBeforeLiquidation = ardentis.market(id).totalBorrowShares;
    params.totalBorrowBeforeLiquidation = ardentis.market(id).totalBorrowAssets;
    params.totalSupplyBeforeLiquidation = ardentis.market(id).totalSupplyAssets;
    ...
    (uint256 returnSeized, uint256 returnRepaid) = ardentis.liquidate(
      marketParams,
      BORROWER,
      amountCollateral,
      0,
    ...
    assertEq(ardentis.position(id, BORROWER).collateral, 0, "collateral");
    ...
    assertEq(
      loanToken.balanceOf(address(ardentis)),
      amountSupplied - amountBorrowed + params.expectedRepaid,
      "ardentis balance"
    );
    assertEq(collateralToken.balanceOf(address(ardentis)), 0, "ardentis collateral balance");
    assertEq(collateralToken.balanceOf(LIQUIDATOR), amountCollateral, "liquidator collateral balance");

    // Bad debt realization.
    assertEq(ardentis.position(id, BORROWER).borrowShares, 0, "borrow shares");
    assertEq(ardentis.market(id).totalBorrowShares, 0, "total borrow shares");
    assertEq(
      ardentis.market(id).totalBorrowAssets,
  ...
  function testBadDebtOverTotalBorrowAssets() public {
    ...
    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(marketParams, collateralAmount, BORROWER, hex"");
    ardentis.borrow(marketParams, loanAmount, 0, BORROWER, BORROWER);
    // Trick to inflate shares, so that the computed bad debt is greater than the total debt of the market.
    ardentis.borrow(marketParams, 0, 1, BORROWER, BORROWER);
    vm.stopPrank();
    ...
    vm.prank(LIQUIDATOR);
    ardentis.liquidate(marketParams, BORROWER, collateralAmount, 0, hex"");
  }
  ...
  function testSeizedAssetsRoundUp() public {
    ...
    vm.prank(LIQUIDATOR);
    (uint256 seizedAssets, uint256 repaidAssets) = ardentis.liquidate(marketParams, BORROWER, 0, 1, hex"");
