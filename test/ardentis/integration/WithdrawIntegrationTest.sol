...
contract WithdrawIntegrationTest is BaseTest {
  ...
  function testWithdrawMarketNotCreated(MarketParams memory marketParamsParamsFuzz) public {
    ...
    ardentis.withdraw(marketParamsParamsFuzz, 1, 0, address(this), address(this));
  }
  ...
  function testWithdrawZeroAmount(uint256 amount) public {
    ...
    ardentis.withdraw(marketParams, 0, 0, address(this), address(this));
  }
  ...
  function testWithdrawInconsistentInput(uint256 amount, uint256 shares) public {
    ...
    vm.expectRevert(bytes(ErrorsLib.INCONSISTENT_INPUT));
    ardentis.withdraw(marketParams, amount, shares, address(this), address(this));
  }
  ...
  function testWithdrawToZeroAddress(uint256 amount) public {
    ...
    ardentis.withdraw(marketParams, amount, 0, address(this), address(0));
  }
  ...
  function testWithdrawUnauthorized(address attacker, uint256 amount) public {
    ...
    ardentis.withdraw(marketParams, amount, 0, address(this), address(this));
  }
  ...
  function testWithdrawInsufficientLiquidity(uint256 amountSupplied, uint256 amountBorrowed) public {
    ...
    vm.prank(SUPPLIER);
    ardentis.supply(marketParams, amountSupplied, 0, SUPPLIER, hex"");

    collateralToken.setBalance(BORROWER, amountCollateral);

    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(marketParams, amountCollateral, BORROWER, hex"");
    ardentis.borrow(marketParams, amountBorrowed, 0, BORROWER, RECEIVER);
    vm.stopPrank();

    vm.prank(SUPPLIER);
    vm.expectRevert(bytes(ErrorsLib.INSUFFICIENT_LIQUIDITY));
    ardentis.withdraw(marketParams, amountSupplied, 0, SUPPLIER, RECEIVER);
  }
  ...
  function testWithdrawAssets(uint256 amountSupplied, uint256 amountBorrowed, uint256 amountWithdrawn) public {
    ...
    (uint256 returnAssets, uint256 returnShares) = ardentis.withdraw(
      marketParams,
      amountWithdrawn,
      0,
      address(this),
    ...
    assertEq(ardentis.position(id, address(this)).supplyShares, expectedSupplyShares, "supply shares");
    assertEq(ardentis.market(id).totalSupplyShares, expectedSupplyShares, "total supply shares");
    assertEq(ardentis.market(id).totalSupplyAssets, amountSupplied - amountWithdrawn, "total supply");
  ...
  function testWithdrawShares(uint256 amountSupplied, uint256 amountBorrowed, uint256 sharesWithdrawn) public {
    ...
    assertEq(ardentis.position(id, address(this)).supplyShares, expectedSupplyShares, "supply shares");
    assertEq(ardentis.market(id).totalSupplyAssets, amountSupplied - expectedAmountWithdrawn, "total supply");
    assertEq(ardentis.market(id).totalSupplyShares, expectedSupplyShares, "total supply shares");
  ...
  function testWithdrawAssetsOnBehalf(uint256 amountSupplied, uint256 amountBorrowed, uint256 amountWithdrawn) public {
    ...
    ardentis.supply(marketParams, amountSupplied, 0, ONBEHALF, hex"");
