...
contract RepayIntegrationTest is BaseTest {
  ...
  function testRepayMarketNotCreated(MarketParams memory marketParamsFuzz) public {
    ...
    ardentis.repay(marketParamsFuzz, 1, 0, address(this), hex"");
  }

  function testRepayZeroAmount() public {
    vm.expectRevert(bytes(ErrorsLib.INCONSISTENT_INPUT));
    ardentis.repay(marketParams, 0, 0, address(this), hex"");
  }
  ...
  function testRepayInconsistentInput(uint256 amount, uint256 shares) public {
    ...
    vm.expectRevert(bytes(ErrorsLib.INCONSISTENT_INPUT));
    ardentis.repay(marketParams, amount, shares, address(this), hex"");
  }

  function testRepayOnBehalfZeroAddress(uint256 input, bool isAmount) public {
    input = bound(input, 1, type(uint256).max);
    vm.expectRevert(bytes(ErrorsLib.ZERO_ADDRESS));
    ardentis.repay(marketParams, isAmount ? input : 0, isAmount ? 0 : input, address(0), hex"");
  }
  ...
  function testRepayAssets(
    ...
    ardentis.borrow(marketParams, amountBorrowed, 0, ONBEHALF, RECEIVER);
    vm.stopPrank();
    ...
    (uint256 returnAssets, uint256 returnShares) = ardentis.repay(marketParams, amountRepaid, 0, ONBEHALF, hex"");
    ...
    assertEq(ardentis.market(id).totalBorrowAssets, amountBorrowed - amountRepaid, "total borrow");
    ...
    assertEq(
      ...
      amountSupplied - amountBorrowed + amountRepaid,
      "ardentis balance"
    );
  }
  ...
  function testRepayShares(
    ...
    (uint256 returnAssets, uint256 returnShares) = ardentis.repay(marketParams, 0, sharesRepaid, ONBEHALF, hex"");
    ...
    assertEq(ardentis.position(id, ONBEHALF).borrowShares, expectedBorrowShares, "borrow shares");
    assertEq(ardentis.market(id).totalBorrowAssets, amountBorrowed - expectedAmountRepaid, "total borrow");
    assertEq(ardentis.market(id).totalBorrowShares, expectedBorrowShares, "total borrow shares");
    assertEq(loanToken.balanceOf(RECEIVER), amountBorrowed, "RECEIVER balance");
    assertEq(
      loanToken.balanceOf(address(ardentis)),
  ...
  function testRepayMax(uint256 shares) public {
    ...
    ardentis.supply(marketParams, 0, shares, SUPPLIER, hex"");
    ...
    vm.prank(BORROWER);
    ardentis.borrow(marketParams, 0, shares, BORROWER, RECEIVER);

    loanToken.setBalance(address(this), assets);

    ardentis.repay(marketParams, 0, shares, BORROWER, hex"");
  }
}
