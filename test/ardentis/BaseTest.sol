...
import { IArdentis } from "ardentis/interfaces/IArdentis.sol";
import "ardentis/interfaces/IArdentisCallbacks.sol";
...
contract BaseTest is Test {
  ...
  function setUp() public virtual {
    ...
    vm.startPrank(OWNER);
    ardentis.enableIrm(address(0));
    ardentis.enableIrm(address(irm));
    ardentis.enableLltv(0);
    ardentis.setFeeRecipient(FEE_RECIPIENT);
  ...
  function _setLltv(uint256 lltv) internal {
    ...
    vm.startPrank(OWNER);
    if (!ardentis.isLltvEnabled(lltv)) ardentis.enableLltv(lltv);
    if (ardentis.market(marketParams.id()).lastUpdate == 0) ardentis.createMarket(marketParams);
    if (ardentis.market(marketParams.id()).fee != 0) ardentis.setFee(marketParams, 0);
    vm.stopPrank();
  ...
  function _supplyCollateralForBorrower(address borrower) internal {
    ...
    vm.startPrank(borrower);
    collateralToken.approve(address(ardentis), type(uint256).max);
    ardentis.supplyCollateral(marketParams, HIGH_COLLATERAL_AMOUNT, borrower, hex"");
    vm.stopPrank();
  }
  ...
  function _boundSupplyCollateralAssets(
    ...
    uint256 collateral = ardentis.position(_id, onBehalf).collateral;
  ...
  function _boundWithdrawCollateralAssets(
    ...
    uint256 collateral = ardentis.position(_id, onBehalf).collateral;
    uint256 collateralPrice = ardentis.getPrice(_marketParams);
    uint256 borrowed = ardentis.expectedBorrowAssets(_marketParams, onBehalf);
  ...
  function _boundSupplyAssets(
    ...
    uint256 supplyBalance = ardentis.expectedSupplyAssets(_marketParams, onBehalf);
  ...
  function _boundSupplyShares(
    ...
    uint256 supplyShares = ardentis.position(_id, onBehalf).supplyShares;
    ...
    return
      bound(
        ...
        0,
        MAX_TEST_AMOUNT
          .toSharesDown(ardentis.market(_id).totalSupplyAssets, ardentis.market(_id).totalSupplyShares)
  ...
  function _boundWithdrawAssets(
    ...
    uint256 supplyBalance = ardentis.expectedSupplyAssets(_marketParams, onBehalf);
    uint256 liquidity = ardentis.market(_id).totalSupplyAssets - ardentis.market(_id).totalBorrowAssets;
  ...
  function _boundWithdrawShares(
    ...
    uint256 supplyShares = ardentis.position(_id, onBehalf).supplyShares;
    uint256 totalSupplyAssets = ardentis.market(_id).totalSupplyAssets;

    uint256 liquidity = totalSupplyAssets - ardentis.market(_id).totalBorrowAssets;
    uint256 liquidityShares = liquidity.toSharesDown(totalSupplyAssets, ardentis.market(_id).totalSupplyShares);
  ...
  function _boundRepayAssets(
    ...
    (, , uint256 totalBorrowAssets, uint256 totalBorrowShares) = ardentis.expectedMarketBalances(_marketParams);
    uint256 maxRepayAssets = uint256(ardentis.position(_id, onBehalf).borrowShares).toAssetsDown(
      totalBorrowAssets,
      totalBorrowShares
    );
  ...
  function _boundRepayShares(
    ...
    uint256 borrowShares = ardentis.position(_id, onBehalf).borrowShares;
  ...
  function _boundLiquidateSeizedAssets(
    ...
    uint256 collateralPrice = ardentis.getPrice(_marketParams);
    uint256 borrowShares = ardentis.position(_id, borrower).borrowShares;
    (, , uint256 totalBorrowAssets, uint256 totalBorrowShares) = ardentis.expectedMarketBalances(_marketParams);
    ...
    uint256 collateral = ardentis.position(_id, borrower).collateral;
  ...
  function _boundLiquidateRepaidShares(
    ...
    uint256 collateralPrice = ardentis.getPrice(_marketParams);
    uint256 maxRepaidAssets = uint256(ardentis.position(_id, borrower).collateral)
  ...
  function _maxBorrow(MarketParams memory _marketParams, address user) internal view returns (uint256) {
    ...
    return
      uint256(ardentis.position(_id, user).collateral).mulDivDown(collateralPrice, ORACLE_PRICE_SCALE).wMulDown(
        _marketParams.lltv
      );
  }
  ...
  function _isHealthy(MarketParams memory _marketParams, address user) internal view returns (bool) {
    ...
    uint256 borrowed = ardentis.expectedBorrowAssets(_marketParams, user);
