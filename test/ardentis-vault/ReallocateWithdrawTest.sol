...
contract ReallocateWithdrawTest is IntegrationTest {
  ...
  function testReallocateWithdrawMax() public {
    ...
    emit EventsLib.ReallocateWithdraw(
      ...
      allMarkets[0].id(),
      CAP2,
      ardentis.position(allMarkets[0].id(), address(vault)).supplyShares
    );
    emit EventsLib.ReallocateWithdraw(
      ALLOCATOR_ADDR,
      allMarkets[1].id(),
      CAP2,
      ardentis.position(allMarkets[1].id(), address(vault)).supplyShares
    );
    emit EventsLib.ReallocateWithdraw(
      ALLOCATOR_ADDR,
      allMarkets[2].id(),
      CAP2,
      ardentis.position(allMarkets[2].id(), address(vault)).supplyShares
    );

    vm.prank(ALLOCATOR_ADDR);
    vault.reallocate(allocations);

    assertEq(ardentis.position(allMarkets[0].id(), address(vault)).supplyShares, 0, "ardentis.supplyShares(0)");
    assertEq(ardentis.position(allMarkets[1].id(), address(vault)).supplyShares, 0, "ardentis.supplyShares(1)");
    assertEq(ardentis.position(allMarkets[2].id(), address(vault)).supplyShares, 0, "ardentis.supplyShares(2)");
  ...
  function testReallocateWithdrawMarketNotEnabled() public {
    ...
    vm.prank(ARDENTIS_OWNER);
    ardentis.createMarket(allMarkets[0]);

    loanToken2.setBalance(SUPPLIER, 1);

    vm.startPrank(SUPPLIER);
    loanToken2.approve(address(ardentis), type(uint256).max);
    ardentis.supply(allMarkets[0], 1, 0, address(vault), hex"");
    vm.stopPrank();
  ...
  function testReallocateWithdrawSupply(uint256[3] memory newAssets) public {
    ...
    (totalSupplyAssets[0], totalSupplyShares[0], , ) = ardentis.expectedMarketBalances(allMarkets[0]);
    (totalSupplyAssets[1], totalSupplyShares[1], , ) = ardentis.expectedMarketBalances(allMarkets[1]);
    (totalSupplyAssets[2], totalSupplyShares[2], , ) = ardentis.expectedMarketBalances(allMarkets[2]);
    ...
    assets[0] = ardentis.expectedSupplyAssets(allMarkets[0], address(vault));
    assets[1] = ardentis.expectedSupplyAssets(allMarkets[1], address(vault));
    assets[2] = ardentis.expectedSupplyAssets(allMarkets[2], address(vault));
  ...
  function testReallocateWithdrawIncreaseSupply() public {
    ...
    emit EventsLib.ReallocateSupply(
      ...
      allMarkets[2].id(),
      3 * CAP2,
      3 * ardentis.position(allMarkets[2].id(), address(vault)).supplyShares
    );
