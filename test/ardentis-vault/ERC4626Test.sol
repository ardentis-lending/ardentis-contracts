...
import { IArdentisFlashLoanCallback } from "ardentis/interfaces/IArdentisCallbacks.sol";

import "./helpers/IntegrationTest.sol";

contract ERC4626Test is IntegrationTest, IArdentisFlashLoanCallback {
  ...
  function testDecimals(uint8 decimals) public {
    ...
    vault = createArdentisVault(OWNER, address(ardentis), address(loanToken), "Ardentis Vault", "MMV");
  ...
  function testMint(uint256 assets) public {
    ...
    assertEq(ardentis.expectedSupplyAssets(allMarkets[0], address(vault)), assets, "expectedSupplyAssets(vault)");
  }
  ...
  function testWithdrawAll(uint256 assets) public {
    ...
    assertEq(ardentis.expectedSupplyAssets(allMarkets[0], address(vault)), 0, "expectedSupplyAssets(vault)");
  }
  ...
  function testWithdrawMoreThanBalanceAndLiquidity(uint256 deposited, uint256 assets) public {
    ...
    ardentis.supplyCollateral(allMarkets[0], type(uint128).max, BORROWER, hex"");
    ardentis.borrow(allMarkets[0], 1, 0, BORROWER, BORROWER);
  ...
  function testMaxWithdraw(uint256 depositedAssets, uint256 borrowedAssets) public {
    ...
    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(allMarkets[0], type(uint128).max, BORROWER, hex"");
    ardentis.borrow(allMarkets[0], borrowedAssets, 0, BORROWER, BORROWER);
  ...
  function testMaxWithdrawFlashLoan(uint256 supplied, uint256 deposited) public {
    ...
    vm.prank(SUPPLIER);
    ardentis.supply(allMarkets[0], supplied, 0, ONBEHALF, hex"");
    ...
    ardentis.flashLoan(address(loanToken), loanToken.balanceOf(address(ardentis)), hex"");
  }
  ...
  function testMaxDeposit() public {
    ...
    vm.prank(SUPPLIER);
    ardentis.supply(allMarkets[0], 1 ether, 0, SUPPLIER, hex"");

    vm.startPrank(BORROWER);
    ardentis.supplyCollateral(allMarkets[0], 2 ether, BORROWER, hex"");
    ardentis.borrow(allMarkets[0], 1 ether, 0, BORROWER, BORROWER);
    vm.stopPrank();
  ...
  function onArdentisFlashLoan(uint256, bytes memory) external view {
