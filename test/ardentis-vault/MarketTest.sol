...
contract MarketTest is IntegrationTest {
  ...
  function testSubmitCapMarketNotCreated(MarketParams memory marketParams) public {
    ...
    vm.assume(ardentis.market(marketParams.id()).lastUpdate == 0);
  ...
  function testEnableMarketWithLiquidity(uint256 deposited, uint256 additionalSupply, uint256 blocks) public {
    ...
    ardentis.supply(allMarkets[3], additionalSupply, 0, address(vault), hex"");
    vm.stopPrank();
    ...
    ardentis.supplyCollateral(allMarkets[0], collateral, BORROWER, hex"");
    ardentis.borrow(allMarkets[0], deposited, 0, BORROWER, BORROWER);
    vm.stopPrank();
