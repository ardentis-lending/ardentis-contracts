...
contract IntegrationTest is BaseTest {
  ...
  // Deploy ArdentisVault from artifacts
  // Replaces using `new ArdentisVault` which would force 0.8.21 on all tests
  // (since ArdentisVault has pragma solidity 0.8.21)
  ...
  ) public returns (IArdentisVault) {
    return newArdentisVault(owner, owner, ardentis, asset, name, symbol);
  }

  function _idle() internal view returns (uint256) {
    return ardentis.expectedSupplyAssets(idleParams, address(vault));
  }
