...
import { IArdentisVaultFactory } from "ardentis-vault/interfaces/IArdentisVaultFactory.sol";
import { ArdentisVault } from "ardentis-vault/ArdentisVault.sol";
import { ArdentisVaultFactory } from "ardentis-vault/ArdentisVaultFactory.sol";
import { ERC20Mock } from "ardentis-vault/mocks/ERC20Mock.sol";
import { TimeLock } from "timelock/TimeLock.sol";

contract ArdentisVaultFactoryTest is Test {
  ...
  function setUp() public {
    ...
    factory = newArdentisVaultFactory();
  }

  function test_createArdentisVault() public {
    (address vaultAddr, address managerTimeLockAddr, address curatorTimeLockAddr) = factory.createArdentisVault(
      admin,
      curator,
      guardian,
      timeLockDelay,
    ...
    ArdentisVault vault = ArdentisVault(vaultAddr);
  ...
  function newArdentisVaultFactory() internal returns (IArdentisVaultFactory) {
    ArdentisVaultFactory impl = new ArdentisVaultFactory(ardentis);
    ERC1967Proxy proxy = new ERC1967Proxy(
      address(impl),
      abi.encodeWithSelector(impl.initialize.selector, admin, vaultAdmin)
    );

    return IArdentisVaultFactory(address(proxy));
  }
}
