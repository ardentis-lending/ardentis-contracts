...
import { Id, MarketParams } from "ardentis/interfaces/IArdentis.sol";

contract ArdentisVaultConfigDeploy is Script {
  using MarketParamsLib for MarketParams;
  // todo update vault
  ArdentisVault TakeVault = ArdentisVault(0x6d6783C146F2B0B2774C1725297f1845dc502525);
  ArdentisVault EGL1Vault = ArdentisVault(0x6d6783C146F2B0B2774C1725297f1845dc502525);
  ...
  function configTake(address deployer, ArdentisVault vault, address whiteList) internal {
  ...
  function configEGL1(address deployer, ArdentisVault vault, address whiteList) internal {
    MarketParams memory BTCBParams = MarketParams({
      loanToken: EGL1,
      collateralToken: BTCB,
      oracle: multiOracle,
  ...
  function configB2(address deployer, ArdentisVault vault, address whiteList) internal {
    MarketParams memory BTCBParams = MarketParams({
      loanToken: B2,
      collateralToken: BTCB,
      oracle: multiOracle,
