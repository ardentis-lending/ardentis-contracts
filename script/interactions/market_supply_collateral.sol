...
contract MarketSupplyCollateral is Script {
  ...
  ERC20Mock collateralTokenContract = ERC20Mock(collateralToken);
  ...
  function run() public {
    ...
    uint256 assets = 10 * 1e18;
    address user = deployer;

    vm.startBroadcast(deployerPrivateKey);

    collateralTokenContract.approve(address(ardentis), type(uint256).max);
    ardentis.supplyCollateral(param, assets, user, hex"");
