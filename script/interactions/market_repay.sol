...
contract MarketRepay is Script {
  ...
  ERC20Mock loanTokenContract = ERC20Mock(loanToken);
  ...
  function run() public {
    ...
    loanTokenContract.approve(address(ardentis), type(uint256).max);
    (, uint128 shares, ) = ardentis.position(id, user);
    ardentis.repay(param, 0, shares, user, hex"");
